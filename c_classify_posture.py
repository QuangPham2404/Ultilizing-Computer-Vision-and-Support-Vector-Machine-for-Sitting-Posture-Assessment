...
model = joblib.load(r"posture_svm_model.pkl")
    ...
    while video_stream.isOpened():
        ...
        if result.pose_landmarks:
            ...
            if calibration_in_process:
                ...
                feature_vector = [ratio_y_nose_shoulders, ratio_y_nose_elbows, ratio_x_nose_shoulders_left, ratio_x_nose_shoulders_right]
                prediction = model.predict(feature_vector)
                feature_vector = [] #reset feature vector for next frame

                # warn user if they are sitting in the wrong posture for more than 10 seconds
                if prediction == 1:
                    incorrect_posture_count += 1
                else:
                    incorrect_posture_10s = False
                if incorrect_posture_count >= 10*fps: #convert 10 seconds into number of frames
                    notify()
                    incorrect_posture_10s = True
                    incorrect_posture_count = 0 # reset
                if incorrect_posture_10s and incorrect_posture_count == 5: 
                    #if the user does not change sitting posture for the next 5s after the warning, notify them again
                    notify()
                    incorrect_posture_10s = True
                    incorrect_posture_count = 0 # reset
